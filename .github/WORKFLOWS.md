...
Push to main
    ...
    │        Syncs public/a-little-more-secure.php Version + README.txt Stable tag & changelog
    │
    │    On PR to main
    └──▶ [pr.yml]
             php -l on 8.2 / 8.3 / 8.4


Merge release PR  →  release-please pushes tag v1.1.0 + creates GitHub Release
    │
    └── v*  ──▶ [wordpress-svn-release.yml]
                    Version check → pack → upload zip to the Release
                    → deploy to WordPress.org SVN (trunk + tags/1.1.0)
```
...
Runs `php -l` over every `*.php` file on a PHP matrix (8.2, 8.3, 8.4). The
...
Push to main
  ...
  release-please
      │
      └──▶ opens / updates PR  "chore(main): release 1.1.0"
                bumps version.txt
...
Release PR opened / updated
    ...
    bash bin/update-plugin-version.sh
              ...
              ├── updates "Version:" header in public/a-little-more-secure.php
...
Tag: v1.1.0
      ...
      │
      ├── bin/pack.sh
      │       copies public/ → build/a-little-more-secure/
      │       zip → a-little-more-secure.zip
      │
      ├──▶ Upload a-little-more-secure.zip to the GitHub Release
...
| `SVN_REPO_URL` | variable | repo | `https://plugins.svn.wordpress.org/a-little-more-secure` |
...
| `public/a-little-more-secure.php` | `bin/update-plugin-version.sh` | `Version:` plugin header |
