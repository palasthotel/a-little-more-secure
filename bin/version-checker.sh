...
VERSION="${VERSION:-}"
if [[ -z "$VERSION" ]]; then
  echo "ERROR: VERSION is not set (e.g. VERSION=1.1.0)" >&2
  exit 1
fi
...
# 3) a-little-more-secure.php plugin header "Version:"
PLUGIN_VERSION="$(grep -E '^[[:space:]]*\*?[[:space:]]*Version:[[:space:]]*[0-9]+\.[0-9]+' "$ROOT_DIR/public/a-little-more-secure.php" \
  | head -n1 \
...
check_eq "a-little-more-secure.php Version" "$PLUGIN_VERSION"
